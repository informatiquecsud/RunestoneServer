...
if db(db.courses.id > 0).isempty():
    ...
    db.courses.insert(course_name='oxocard101', base_course='oxocard101',
                      term_start_date=datetime.date(2019, 9, 1))
    db.courses.insert(course_name='ipt-prog-3', base_course='ipt-prog-3',
                      term_start_date=datetime.date(2019, 9, 1))
else:
...
if environ.get('WEB2PY_MIGRATE', "") != 'fake':
    click.echo(message="Adding Constraints and Indices",
               file=None, nl=True, err=False, color='red')
    try:
        db.executesql(
            '''alter table questions add constraint name_bc_unique UNIQUE(name, base_course)''')
        db.executesql(
        ...
        click.echo(message="The creation of one or more indices/constraints failed",
                   file=None, nl=True, err=False, color='red')
        sys.exit(1)
